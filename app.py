...
def image_to_ply_with_color(image_path, ply_output, mesh_style="realistic"):
    ...
    # Calculate intensity using the weighted formula
    intensity = 0.2989 * R + 0.5870 * G + 0.1140 * B

    if mesh_style == "lowpoly":
        reduction_factor = 10
        intensity = intensity[::reduction_factor, ::reduction_factor]
        img_array = img_array[::reduction_factor, ::reduction_factor]
        x, y = np.meshgrid(np.linspace(0, width - 1, intensity.shape[1]),
                           np.linspace(0, height - 1, intensity.shape[0]))
        x = x.flatten()
        y = y.flatten()

    # Normalize and scale intensity to use as height (z-axis)
    z = intensity.flatten()
    ...
    for i in range(rows - 1):
        for j in range(cols - 1):
            ...
            if j == 0:
                vt = i * cols + j
                vb = bottom_offset + i * cols + j
                vtn = (i + 1) * cols + j
                vbn = bottom_offset + (i + 1) * cols + j
                faces.append([vt, vb, vtn])
                faces.append([vb, vbn, vtn])
            # Back
            if j == cols - 2:
                vt = i * cols + j + 1
                vb = bottom_offset + i * cols + j + 1
                vtn = (i + 1) * cols + j + 1
                vbn = bottom_offset + (i + 1) * cols + j + 1
                faces.append([vtn, vb, vt])
                faces.append([vtn, vbn, vb])
            # Left
            if i == 0:
                vt = i * cols + j
                vb = bottom_offset + i * cols + j
                vtn = i * cols + j + 1
                vbn = bottom_offset + i * cols + j + 1
                faces.append([vt, vb, vtn])
                faces.append([vb, vbn, vtn])
            # Right
            if i == rows - 2:
                vt = (i + 1) * cols + j
                vb = bottom_offset + (i + 1) * cols + j
                vtn = (i + 1) * cols + j + 1
                vbn = bottom_offset + (i + 1) * cols + j + 1
                faces.append([vtn, vb, vt])
                faces.append([vtn, vbn, vb])
